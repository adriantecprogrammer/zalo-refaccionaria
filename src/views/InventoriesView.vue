...
const handleBarcodeInput = async () => {
  const product = await $inventoryTransferStore.getProductByBarcode(
    barcodeInput.value,
  );
  if (product) {
...
watch(
  () => $inventoryTransferStore.selectedOrigin,
  () => {
...
watch(
  () => $inventoryTransferStore.selectedDestination,
  () => {
...
<template>
  <DefaultLayout>
      ...
      <div class="mt-5 p-5 xl:p-10">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody>
              <tr
                v-for="product in $inventoryTransferStore.transferHistory"
                :key="product.id"
              ...
              >
                ...
                <td scope="row" class="px-6 py-4">
                  {{ product.fromStorehouseId === 1 ? "Fala" : "Bodega" }}
                </td>
                <td class="px-6 py-4">
                  {{ product.fromStorehouseId === 1 ? "Fala" : "Bodega" }}
                </td>
      ...
      <div class="flex flex-col p-5 gap-y-5 xl:p-10 max-w-screen-sm">
        <div class="flex gap-x-5">
          <div>
            ...
            <p class="text-sm">
              Elige el <strong>Origen</strong> de los productos a mover
            </p>
            <select
              v-model="$inventoryTransferStore.selectedOrigin"
              class="border-2 rounded-md border-secondary-900 mt-2 p-2 w-full"
            >
              <option
                v-for="warehouse in $inventoryTransferStore.availableOrigins"
                :key="warehouse.id"
                :value="warehouse"
              >
                {{ warehouse.name }}
            ...
            <button
              v-if="$inventoryTransferStore.selectedOrigin"
              @click="clearSelectedOrigin"
          ...
          <div>
            ...
            <p class="text-sm">
              Elige el <strong>Destino</strong> para el traslado de los
              productos
            </p>
            <select
              v-model="$inventoryTransferStore.selectedDestination"
              class="border-2 rounded-md border-secondary-900 mt-2 w-full p-2"
            >
              <option
                v-for="warehouse in $inventoryTransferStore.availableDestinations"
                :key="warehouse.id"
                :value="warehouse"
              >
                {{ warehouse.name }}
            ...
            <button
              v-if="$inventoryTransferStore.selectedDestination"
        ...
        <div class="mt-3">
          <input
            placeholder="Introduce el código de barras del producto a mover"
            type="text"
            class="border-2 rounded-md border-secondary-900 p-2 w-full"
          ...
          <p
            ...
            class="text-xs mt-1"
          >
            Debes seleccionar el origen y el destino para poder introducir el
            código de barras.
          </p>
        </div>
      </div>
      <div class="mt-5 p-5 xl:p-10">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3">
                  Cantidad a la mano (Stock Quantity)
                </th>
            ...
            <tbody>
              <tr
                v-for="product in $inventoryTransferStore.productsToTransfer"
                :key="product.name"
                class="bg-white border-b hover:bg-gray-50"
              >
                <th
                ...
                <td class="px-6 py-4">
                  <span
                    :class="{
                      'text-green-500':
                        product.storehouseId !==
                        $inventoryTransferStore.selectedDestination?.id,
                      'text-red-500':
                        product.storehouseId ===
                        $inventoryTransferStore.selectedDestination?.id,
                    }"
        ...
        <button
          ...
          :class="{
            'bg-primary-600 cursor-not-allowed':
              $inventoryTransferStore.emptyProducts,
            'bg-primary-900 hover:bg-primary-1000':
              !$inventoryTransferStore.emptyProducts,
          }"
          :disabled="$inventoryTransferStore.emptyProducts"
          @click="handleConfirm"
        >
          Confirmar Traslado
        </button>
    ...
    <ConfirmDialog
      :visible="showDialog"
      :quantity="$inventoryTransferStore.productsToTransfer.length"
      @close="closeDialog"
