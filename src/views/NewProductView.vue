...
const handleSubmit = async (event: Event) => {
  ...
  if (barcode.value.includes(" ")) {
    $notificationStore.showNotification(
      "El código de barras no debe contener espacios",
      "error",
    );
    return;
  }
        ...
        <section>
          <form @submit="handleSubmit" class="max-w-md mx-auto">
            <div class="relative z-0 w-full mb-5 group">
              ...
              <label
                for="name"
                class="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 peer-focus:text-primary-1000 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                >Nombre</label
              >
            </div>
            ...
            <div class="mb-5">
              ...
              <ul
                class="h-48 pr-3 pb-3 overflow-y-auto text-sm text-gray-700"
            ...
            <div class="relative z-0 w-full mb-5 group">
              ...
              <select
                ...
                class="border-2 rounded-md border-secondary-900 w-full"
                required
              >
                <option
                  v-for="warehouse in $inventoryTransferStore.warehouses"
                  :key="warehouse.id"
                  :value="warehouse.id"
                >
                  {{ warehouse.name }}
            ...
            <button
              type="submit"
              class="mt-5 text-white bg-primary-900 hover:bg-primary-1000 focus:ring-4 focus:outline-none focus:ring-primary-700 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center"
            >
              Agregar Producto
            </button>
          </form>
