...
const goToHomeView = () => { router.push('/home') }
...
<template>
    <main class="lg:w-3/4 mx-auto p-10 space-y-10 m-6">
        ...
        <div v-else>
            <ItemProductProcessDialog :visible="showProducts" :cart="cart" @close="closeItemProductProcessDialog">
            ...
            <table class="mt-4 w-full text-sm text-center text-gray-500">
                ...
                <tbody>
                    <tr v-for="cart in carts" :key="cart.id" class="bg-white border-b hover:bg-gray-50">
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">Venta {{ cart.id }}
                        </td>
                        <td>{{ cart.createdAt }}</td>

                        <button @click="showItemProductProcessDialog(cart.id)"
                            class=" boton_detalle ml-8 px-2 py-2 mt-2 mb-2 text-white bg-primary-900 hover:bg-primary-1000 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm text-nowrap text-center">Ver
                            Detalles</button>
                    </tr>
                </tbody>
            </table>
