...
const pauseSail = () => {
  ...
  $cartStore.postSaleInProcess(cartJson)
    .then(response => {
      if (response.success) {
...
<template>
  ...
  <DefaultLayout>
    <transition name="fade">
      <ProductSearchAndCart v-show="showProductSearch" @close="showProductSearch = false" />
    </transition>
    ...
    <div v-show="!showProductSearch" class="relative lg:h-screen lg:flex ">
        ...
        <section class="space-y-4">
          ...
          <div class="relative">
            ...
            <input type="search" id="search"
              ...
              placeholder="Buscar por nombre" required @click="showProductSearch = true" />
          </div>
        </section>
      </main>
      ...
      <aside class="lg:w-3/5 bg-white border border-l-2 px-5 py-10 flex flex-col lg:h-screen">
        <div class="flex-col">
          ...
          <div class="flex-row">
            <button @click="pauseSail" v-if="showBotonPause"
              class="boton_detalle px-4 py-2 mt-1 mb-4 text-white bg-primary-900 hover:bg-primary-1000 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm text-nowrap text-center">
              <i class="fas fa-pause mr-2"></i>
              Pausar venta
            </button>
            <small v-if="peticion" class=" scale-in-center ml-4 text-blue-600">Venta pausada</small>
            <small v-if="errorPetition" class=" scale-in-center ml-4 text-red-600 font-bold">Ocurrio un error al
              pausar</small>
            <Toast />
          </div>
        ...
        <section class="flex-grow lg:overflow-y-auto mb-10" v-auto-animate>
          <ProductCart v-for="product in $cartStore.cart" :key="product.barcode" :product="product" />
        </section>

        <div class="flex items-end mb-2">
          <div v-if="isValidPromoCode && applyPromoValidation"
            class="p-1  text-xs text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
            role="alert">
            <span class="font-light">Descuento aplicado {{ promoCodeDiscount }}%</span>
          </div>

          <div v-if="!isValidPromoCode && applyPromoValidation"
            class="p-1  text-xs text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-green-400" role="alert">
        ...
        <section class="mt-auto space-y-4">
          <div class="flex justify-between  ">
            ...
            <div v-if="showCuppon" class="flex mr-1 ">
              ...
              <transition name="fade">
                <GreenCheckMark :discount="promoCodeDiscount" v-if="isValidPromoCode && applyPromoValidation" />
              </transition>
          ...
          <button class="w-full uppercase px-4 py-2 text-center text-white rounded-lg"
            @click="$router.push({ name: ROUTE_NAME.PAYMENT_VIEW }); reSetDisableButton()"
            :disabled="$cartStore.emptyCart"
            :class="{ 'bg-primary-600 cursor-not-allowed': $cartStore.emptyCart, 'bg-primary-900 hover:bg-primary-1000': !$cartStore.emptyCart }">
            Ir a pagar
          </button>
        </section>
      </aside>
    ...
    <OpenShiftDialog :visible="showCalculatorModal" @close="handleClose" @confirm="handleConfirm" />
