...
const sortFilters = ref<{ recent: boolean; oldest: boolean }>({ recent: false, oldest: false });
...
watch([sortFilters, selectedCashier], () => {
  applyFilters();
}, { deep: true });
...
const applyFilters = () => {
  ...
  if (selectedCashier.value !== null) {
    sortedOrders = sortedOrders.filter(order => order.userId === selectedCashier.value);
  }
...
const handleFilterChange = ({ filter, value }: { filter: 'recent' | 'oldest'; value: boolean }) => {
  if (filter === 'recent') {
  ...
  <DefaultLayout>
    <div>
      <main class="lg:w-3/4 mx-auto p-10 space-y-10">
        <DetailsSaleDialog :visible="showDetailsSale" :order="selectedOrder" :orderDetail="orderProducts"
          :cashier="selectedOrder ? getUserNameById(selectedOrder.userId) : ''" @close="closeDetailsSaleModal" />
        ...
        <section>
          <div class="flex flex-row-reverse items-center gap-2">
            ...
            <select v-model="selectedCashier" class="border border-gray-300 rounded-lg p-2">
          ...
          <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-5">
            <table class="w-full text-sm text-left text-gray-500">
              ...
              <tbody>
                <tr v-for="order in filteredOrders" :key="order.id" class="bg-white border-b hover:bg-gray-50">
                  <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ getUserNameById(order.userId) }}
                  </td>
                  ...
                  <button @click="showDetailsSaleModal(order)"
                    class=" boton_detalle px-2 py-2 mt-2 mb-2 text-white bg-primary-900 hover:bg-primary-1000 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm text-nowrap text-center">Ver
                    Detalles</button>
                </tr>
              </tbody>
            </table>
