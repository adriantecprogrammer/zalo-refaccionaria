...
const imprimirTicket = async (id: number | undefined) => {
  try {
    ...
    const response = await axios.get(urlEndpoint, { headers, responseType: 'blob' });
...
<template>
  <div v-if="visible" class="card" id="confirm-modal" tabindex="-1" aria-hidden="true">
        ...
        <!-- Modal content -->
          ...
          <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
              <h3 class="text-3xl font-semibold text-gray-900">Detalle de Venta</h3>
              <button type="button"
            ...
            <div class="p-4 md:p-5 space-y-4">
              <p class="text-base leading-relaxed text-gray-600">Cajero: <span class="font-bold">{{ cashier }}</span> </p>
              <p class="text-base leading-relaxed text-gray-600">Fecha:  <span class="font-bold">{{ order?.date }}</span></p>
              <p class="text-base leading-relaxed text-gray-600">Método de Pago: <span class="font-bold"> {{ order?.paymentMethod }}</span> </p>
              <p class="text-2xl font-bold leading-relaxed text-gray-900"> Productos </p>
              <div v-for="products in props.orderDetail" :key="products.id">
                <p class="text-base leading-relaxed text-gray-600 xl font-bold">- {{ products.productName }} </p>
                <div class="flex-row">
              ...
              <p class="text-2xl font-bold leading-relaxed text-gray-900">Total: ${{ order?.total }}</p>

            </div>

            <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b">
              <button @click="imprimirTicket(order?.id)" type="button"
