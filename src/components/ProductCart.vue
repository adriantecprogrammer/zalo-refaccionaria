...
const price = computed(() => {
  ...
  return (
    ($props.product.salePrice - discountAmount) * productQuantityInCart.value
  );
});
...
const applyPromoCode = () => {
  ...
  if (discountCodes.value) {
    for (; index < discountCodes.value.length; index++) {
      ...
      if (element.value) {
        if (
          element.value.code === promoCode.value &&
          element.value.isActive === 1
        ) {
    ...
    if (product) {
      const newPrice =
        product.salePrice - (product.salePrice * promoCodeDiscount.value) / 100;
...
<template>
    ...
    >
      ...
      <div class="col-span-3 justify-self-start space-y-2">
        ...
        <span
          ...
          :class="[
            product.storehouseId === 1
              ? 'bg-primary-600 text-primary-900'
              : 'bg-secondary-600 text-secondary-900',
          ]"
        >
      ...
      >
        <select
          ...
          v-model="discount"
          class="p-2 border rounded lg:m-5"
        >
        ...
        <CloseIcon
          @click="$cartStore.deleteItem(product)"
  ...
  <div v-if="showCuppon" class="flex justify-end mr-1 mt-2">
    ...
    <input
      type="text"
      id="promoCodeInput"
      class="items-end block w-36 p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-yellow-200 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
    ...
    <button
      ...
      type="button"
      class="focus:outline-none text-white bg-primary-1000 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm ml-1 px-2 py-1 me-2 dark:focus:ring-yellow-900"
    >
      Aplicar
    </button>
  </div>
