...
export const useInventoryTransferStore = defineStore('inventoryTransferStore', () => {
  
  const emptyProducts = computed<boolean>(() => productsToTransfer.value.length === 0);
  ...
  const availableOrigins = computed(() => {
    return warehouses.value.filter(warehouse => warehouse.id !== selectedDestination.value?.id);
  });

  const availableDestinations = computed(() => {
    return warehouses.value.filter(warehouse => warehouse.id !== selectedOrigin.value?.id);
  });
  ...
  const loadTransfersFromStorage = () => {
    const storedTransfers = localStorage.getItem(getUserSpecificStorageKey('transfer'));
    productsToTransfer.value = storedTransfers ? JSON.parse(storedTransfers) : [];
  };

  const loadSelectedOriginFromStorage = () => {
    const storedOrigin = localStorage.getItem(getUserSpecificStorageKey('selectedOrigin'));
    selectedOrigin.value = storedOrigin ? JSON.parse(storedOrigin) : null;
  };

  const loadSelectedDestinationFromStorage = () => {
    const storedDestination = localStorage.getItem(getUserSpecificStorageKey('selectedDestination'));
    selectedDestination.value = storedDestination ? JSON.parse(storedDestination) : null;
  };

  const saveSelectedOriginToStorage = () => {
    localStorage.setItem(getUserSpecificStorageKey('selectedOrigin'), JSON.stringify(selectedOrigin.value));
  };

  const saveSelectedDestinationToStorage = () => {
    localStorage.setItem(getUserSpecificStorageKey('selectedDestination'), JSON.stringify(selectedDestination.value));
  };
  ...
  watch(productsToTransfer, (newTransfers) => {
    localStorage.setItem(getUserSpecificStorageKey('transfer'), JSON.stringify(newTransfers));
  }, { deep: true });

  const addProductToTransfer = (product: Product) => {
    if (!productsToTransfer.value.some(p => p.barcode === product.barcode)) {
      productsToTransfer.value.push(product);
    }
  };

  const removeProductFromTransfer = (id: number) => {
    productsToTransfer.value = productsToTransfer.value.filter(p => p.id !== id);
  };
  ...
  const registerTransfer = async () => {
    if (!selectedOrigin.value || !selectedDestination.value) {
      notificationStore.showNotification('Origen y destino son requeridos', 'error');
      return;
    }
    ...
    const payload = {
      ...
      status: 'Pendiente',
      products: productsToTransfer.value.map(product => ({ id: product.id })),
    };

    try {
      const response = await apiClient.post('/transfers', payload);
      clearTransfers();
      notificationStore.showNotification('Transferencia registrada con éxito', 'success');
      return response.data;
    } catch (error) {
      console.error('Error registering transfer:', error);
      notificationStore.showNotification('Error al registrar la transferencia', 'error');
      throw error;
    }
  };
    ...
    try {
      ...
      if (product) {
        if (product.stockQuantity > 0) {
          if (selectedOrigin.value && product.storehouseId === selectedOrigin.value.id) {
            return product;
          } else {
            notificationStore.showNotification('El producto seleccionado no se encuentra en el origen seleccionado', 'error');
            return null;
          }
        } else {
          notificationStore.showNotification('No hay stock suficiente o el producto no existe', 'error');
          return null;
        }
      } else {
